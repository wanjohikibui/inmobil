...
jQuery.fn.ingrid = function(o){
	...
	var cfg = {
		...
		initalLoad : false,							// when Ingrid is initialized, should it load data immediately?
				...
				success: function(result){						
					...
					if (cfg.dataType == 'html') {
						...
						var row  = $tbl.find('tr:first');
							...
							alert('Error: Total columns returned [' + table.find('tbody tr:first td').length + '] do not match Ingrid ['+ cfg.colWidths.length +'].');
						}
					}
					if (cb) cb();
				},
		...
		*/
		...
		if (cfg.savedStateLoad && jQuery.cookie) {
			...
			if (!param_str) {
				...
				if (hash['page'].toLowerCase() != 'undefined') {
